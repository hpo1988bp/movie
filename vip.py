...
plugin = Plugin()
addon = xbmcaddon.Addon("plugin.video.vip.playlist")
pluginrootpath = "plugin://plugin.video.vip.playlist"
	...
	for row in _json["table"]["rows"]:
		...
		if "plugin://" in item["path"]:
			if "install-repo" in item["path"]:
				item["is_playable"] = False
			elif re.search("plugin.video.vip.playlist/(.+?)/.+?\://", item["path"]):
				match = re.search(
					"plugin.video.vip.playlist(/.+?/).+?\://", item["path"])
...
def AddTracking(items):
	...
	for item in items:
		if "plugin.video.vip.playlist" in item["path"]:
